...
public class Trainable_Segmentation_EX implements PlugIn 
{
	...
	private int sigmaMin=1;
	private int sigmaMax=16;
	...
	private static final int MAX_NUM_CLASSES = 10;
	...
	private final Color[] colors = new Color[]{Color.green, Color.blue,
			Color.cyan, Color.orange, Color.red,Color.black,Color.orange, Color.gray, Color.magenta,Color.white};
	/** names of the current classes */
	private String[] classLabels = new String[]{"class 1", "class 2", "class 3", "class 4", "class 5", "class 6", "class 7", "class 8", "class 9", "class 10"};
	 ...
	 * Basic constructor
	 */
	public Trainable_Segmentation_EX() 
	{
		this.useGUI = true;
		...
		examplesLoadButton = new JButton ("Load examples");
		examplesLoadButton.setToolTipText("Load examples from a previous expertise session");
		examplesLoadButton.setEnabled(true);
		
		examplesSaveButton = new JButton ("Save examples");
		examplesSaveButton.setToolTipText("Save examples from a previous expertise session");
		examplesSaveButton.setEnabled(true);
...
//		setButtonsEnabled(true);
	...
	public PointRoi [][] convertExamplesToSlicePointRoi() {
		...
		Roi[][]tabRoi=new Roi[nClasses][];
		//Derniere brique : lire les roi de this, et en faire un beau tableau
		for(int cl=0;cl<nClasses;cl++) {
			tabRoi[cl]=new Roi[this.examples[cl].size()];
			System.out.println("Conversion classe "+cl);
			System.out.println("nb "+tabRoi[cl].length);
			for(int ex=0;ex<tabRoi[cl].length;ex++) {
				System.out.println("example "+ex+" contient "+this.examples[cl].get(ex).getContainedPoints().length);
				tabRoi[cl][ex]=this.examples[cl].get(ex);
			}
		}
		
		int zMax=this.displayImage.getStackSize();
		PointRoi [][]prOut=new PointRoi[nClasses][zMax];
		for(int cl=0;cl<nClasses;cl++) {
			for(int z=1;z<=zMax;z++) {
				prOut[cl][z-1]=new PointRoi();
				for(int ro=0;ro<tabRoi[cl].length;ro++) {
					System.out.println("Evaluation appartenance à la slice "+z);
					System.out.println("Et justement, z="+tabRoi[cl][ro].getZPosition());
					if(tabRoi[cl][ro].getZPosition()==z-1) {
						Point[] ptTab=tabRoi[cl][ro].getContainedPoints();
						for(int pt=0;pt<ptTab.length;pt++) {
							prOut[cl][z-1].addPoint(ptTab[pt].x,ptTab[pt].y);
						}
					}
				}
			}
	...
	public void saveExamplesInFile(PointRoi[][]prTab) {		
		...
		for(int cl=0;cl<prTab.length;cl++) {
			writer.println(this.classLabels[cl]);
		}
		writer.close();	
		return;
	}
	...
	public void loadExamplesFromFile(){
		...
		try{
			...
			for(int i=0;i<nClasses;i++)this.classLabels[i]=buff.readLine();
			buff.close(); 
		}		
		catch (Exception e){}
		...
		//Reactualize array of roi and classes
		//class already here, assuming that the new number of classes, nClasses is upper or equal to the older numOfClasses
		for(int i=0;i<numOfClasses;i++) {
			examples[i]=new ArrayList<Roi>();
		...
		for(int i = numOfClasses; i < nClasses ; i++)		{
			addNewClass(classLabels[i]);
		}		
		
		traceCounter=new int[this.MAX_NUM_CLASSES];

		
		System.out.println("Apres nettoyage actuel. Integration nouveaux examples");
		for(int cl=0;cl<numOfClasses;cl++) {
			System.out.println("Traitement classe "+cl+" parmi "+numOfClasses);
			
			for(int sl=0;sl<nSlices;sl++) {
				System.out.println("Traitement slice "+sl+" qui contient : "+prTab[cl][sl].getContainedPoints().length);
				if(prTab[cl][sl].getContainedPoints().length>0) {
					System.out.println("Ajout d un exemple");
					addExamplesBis(prTab[cl][sl],cl,sl+1);
					System.out.println("Exemple ajoute");
				}
			}
		}
		this.win.examplesJPanel.revalidate();
		this.win.examplesJPanel.repaint();
		win.pack();
		setButtonsEnabled(true);
		this.repaintWindow();
		return;
		...
		public void actionPerformed(final ActionEvent e) {
			...
			exec.submit(new Runnable() {
				...
				{											
					if(e.getSource() == trainButton)
					{
					...
					else if(e.getSource() == examplesLoadButton){
						loadExamples();
					}
						...
						System.out.println("Detecte une demande de nouvelle classe");
						addNewClass();
					}
...
/////////////////////////////////////////////////////
		private JPanel examplesJPanel=new JPanel();
/////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////
			// Examples panel
						examplesJPanel.setBorder(BorderFactory.createTitledBorder("Examples"));
						GridBagLayout examplesLayout = new GridBagLayout();
						GridBagConstraints examplesConstraints = new GridBagConstraints();
						examplesConstraints.anchor = GridBagConstraints.NORTHWEST;
						examplesConstraints.fill = GridBagConstraints.HORIZONTAL;
						examplesConstraints.gridwidth = 1;
						examplesConstraints.gridheight = 1;
						examplesConstraints.gridx = 0;
						examplesConstraints.gridy = 0;
						examplesConstraints.insets = new Insets(5, 5, 6, 6);
						examplesJPanel.setLayout(examplesLayout);
						
						examplesJPanel.add(examplesSaveButton, examplesConstraints);
						examplesConstraints.gridy++;
						examplesJPanel.add(examplesLoadButton, examplesConstraints);
						examplesConstraints.gridy++;
/////////////////////////////////////////////////////
			...
			buttonsPanel.add(examplesJPanel, buttonsConstraints);
		...
		{						
			System.out.println("Toto 5 Detecte une demande de nouvelle classe");
			...
			System.out.println("Toto 6 Detecte une demande de nouvelle classe");
			...
			annotationsConstraints.gridy++;
			System.out.println("Toto 7 Detecte une demande de nouvelle classe");
			...
			repaintAll();
			System.out.println("Toto 8 Detecte une demande de nouvelle classe");
	...
	public void run(String arg) 
	{
		...
		featureStack.setMaximumSigma(this.sigmaMax);
		featureStack.setMinimumSigma(this.sigmaMin);
	...
	private void addNewClass() {
		System.out.println("Toto 1 Detecte une demande de nouvelle classe");

		String inputName = JOptionPane.showInputDialog("Please input a new label name");
		
		if(null == inputName)
			return;
		else addNewClass(inputName); 

	}
		
	private void addNewClass(String inputName) 
	{
	...
	public boolean showSettingsDialog()
	{
		...
		gd.addMessage("Sigmas:");
		gd.addNumericField("sigma min:", sigmaMin, 0);
		gd.addNumericField("sigma max:", sigmaMax, 0);
		...
		gd.addButton("Save feature stack", new ButtonListenerSaveFeatureStack("Select location to save feature stack", featureStack));
		...
		final int newSigmaMin = (int) gd.getNextNumber();
		final int newSigmaMax = (int) gd.getNextNumber();
		if( (this.sigmaMin != newSigmaMin) || (this.sigmaMax != newSigmaMax) )featuresChanged=true;
		this.sigmaMin=newSigmaMin;
		this.sigmaMax=newSigmaMax;
		this.featureStack.setMaximumSigma(this.sigmaMax);
		this.featureStack.setMinimumSigma(this.sigmaMin);
	 ...
	 * settings dialog 
	 */
	static class ButtonListenerSaveFeatureStack implements ActionListener 
	{
		private String title;
		private TextField text;
		private FeatureStack featureStack;

		public ButtonListenerSaveFeatureStack(String title, FeatureStack featureStack) 
		{
			this.title = title;
	 ...
	 */
	public Trainable_Segmentation_EX(ImagePlus trainingImage) 
	{	
		// no GUI
		this.useGUI = false;
