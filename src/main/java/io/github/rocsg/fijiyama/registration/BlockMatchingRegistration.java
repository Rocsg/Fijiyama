...
//		setupAndRunRsmlBlockMatchingRegistration("/home/rfernandez/Bureau/A_Test/BPMP/Reproduction_01_avec_reseau_morgan/Train/20200826-AC-PIP_azote_Seq 6_Boite 00005_IdentificationFailed-Visu.jpg");
    ...
    public static int logNumberOfBlocks(int inN) {
        int outN = 300;
        if (inN > 1000000) {
            outN += (inN - 1000000) * 0.0001;
            inN -= (inN - 1000000);
        }
        if (inN > 100000) {
            outN += (inN - 100000) * 0.0007;
            inN -= (inN - 100000);
        }
        if (inN > 10000) {
            outN += (inN - 10000) * 0.009;
            inN -= (inN - 10000);
        }
        if (inN > 4000) {
            outN += (inN - 4000) * 0.07;
    ...
    public static double[] estimateBlocksNumberPerLevel(int[] dims, int viewRegistrationLevel, int levelMin, int levelMax, int nbIter, Transform3DType transformType,
        ...
        for (int lev = levelMax; lev >= levelMin; lev--) {
            ...
            int nbBlocksZ = 1;
            if (dims[0] > 1)
                nbBlocksX = Math.round(1 + (dims[0] / factorDiv[0] - blockHalfSizes[0] - 2 * neighbourhoods[0] * strideLev[0]) / strideLev[0]);
            if (dims[1] > 1)
                nbBlocksY = Math.round(1 + (dims[1] / factorDiv[1] - blockHalfSizes[1] - 2 * neighbourhoods[1] * strideLev[1]) / strideLev[1]);
            if (dims[2] > 1)
                nbBlocksZ = Math.round(1 + (dims[2] / factorDiv[2] - blockHalfSizes[2] - 2 * neighbourhoods[2] * strideLev[2]) / strideLev[2]);
            int nbBlocksTotal = nbBlocksX * nbBlocksY * nbBlocksZ;
            tabNb[levelMax - lev] = nbBlocksTotal * 1.0 * percentageVarianceSelect / (1E2);
        }
        return tabNb;
    }
...
*/
    ...
    public ImagePlus displayDistanceMapOnGrid(ItkTransform tr, ImagePlus grid) {
        ...
        ImagePlus maskRoot = VitimageUtils.getBinaryMask(null, 10); // ImagePlus maskRoot = VitimageUtils.getBinaryMask(plongement(this.imgRef, this.rootModel, false), 10);
...
//		ImagePlus maskOutGrid=VitimageUtils.getBinaryMaskUnary(gridTemp, 1);
    ...
    public int[][] imageDimsAtSuccessiveLevels(int[] dimsImg, int[] subFactors, boolean noSubScaleZ) {
        ...
        if (noSubScaleZ) for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < 3; j++) ret[i][j] = (int) Math.ceil(dimsImg[j] / subFactors[i]);
        return ret;
    }
    ...
    public double[][] imageVoxSizesAtSuccessiveLevels(double[] voxSizes, int[] subFactors) {
        ...
        if (noSubScaleZ) {
            ...
            for (int i = 0; i < n; i++) System.arraycopy(voxSizes, 2, ret[i], 2, 1);
